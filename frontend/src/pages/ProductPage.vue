...
const formatter = new Intl.NumberFormat(navigator.language, {
   style: 'currency',
   currency: (inject('ui') as UI).currency,
});
...
const makePhotoURL = (filename: string | undefined) => filename ? `${storageURL}/${filename}` : '';
...
const decrease = async () => {
   ...
   const originalCount = count.value;
   count.value--;
   try {
      const cartId = cartItem.value?.id;
      if (!cartId) return;

      let response;

      if (count.value === 0) {
         response = await CartService.destroy(cartId);
         if (response.success) {
            ElMessage.success('Удалено из корзины!');
            userStore.removeCartItem(cartId);
            cartItem.value = undefined;
         } else {
            response = await CartService.update(cartId, { quantity: count.value });
            if (response.success) {
               userStore.updateCartItem(cartId, count.value);
            }
         }
      ...
      count.value = originalCount;
      ElMessage.error(error instanceof Error ? error.message : 'Ошибка обновления корзины');
      console.error(error);
   }
}
   ...
   if (userStore.isAuth && product.id) {
      ...
      if (response.success) {
         if (response.message === 'added') {
            ...
            userStore.removeFromFavorite(product.id);
